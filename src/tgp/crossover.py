...
def __slice_genome(
    genome, 
    partner_gene_length=None, 
    replacement_gene_length=None
):
    gene_type = gen.rng.choice([1, 0], p=[.9, .1]) # 1 = Non-Leaf, 0 = Leaf
    ...
    for idx, gene in enumerate(genome):
        if (gene_type and 'value' not in gene) or (not gene_type and 'value' in gene):
            # check if the size of this node exceeds size_cap
            # TODO: [-1] only sees the start of the neighbour, no clue about size of subtree
            gene_size = (gene['args'][-1] - gene['args'][0]) + 1 if gene_type else 1

            if partner_gene_length and replacement_gene_length:
                partner_bloat_control = gene_size + (partner_gene_length - replacement_gene_length) >= cf.BLOAT_CONTROL
                genome_bloat_control = (genome_length - gene_size) + replacement_gene_length >= cf.BLOAT_CONTROL

                if partner_bloat_control or genome_bloat_control:
                    continue
...
def __combine_genome(parent_splice_sequence, parent, genome_sequence):
    ...
    # print(prefix_parent, '-->', postfix_parent)
    ...
    # need to walk list and check arguements of splice 1 and make sure they are pointing 
    # to the right location if its > than len of the prefix gene sequence
...
def recombination(parent_1, parent_2):
    # hard upper bound 64 (around depth of 6 in a full binary tree) to avoid bloat
    ...
    partner_gene_length = len(parent_1)
    replacement_gene_length = len(genome_slice_p1)

    p2_splice, genome_slice_p2 = __slice_genome(
        parent_2, 
        partner_gene_length,
        replacement_gene_length)

    # print(p1_splice, genome_slice_p1)
    # print(p2_splice, genome_slice_p2)

    # print(f'Length: {len(parent_1)} Parent: {parent_1}')
    # print(f'Length: {len(parent_2)} Parent: {parent_2}')
    ...
    # print(f'Length: {len(child_1_genome)} Offspring: {child_1_genome}')
    # print(f'Length: {len(child_2_genome)} Offspring: {child_2_genome}')
